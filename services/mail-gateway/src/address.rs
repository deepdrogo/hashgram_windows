...
pub fn classify(addr: &str, our_domain: &str) -> Result<Mailbox, GatewayError> {
    ...
    let raw = raw.rsplit_once(':').filter(|(l, _)| l.starts_with('@')).map_or(raw, |(_, r)| r);
    if raw.is_empty() || raw.len() > 254 {
        return Err(GatewayError::Address(format!("bad address length: {}", raw.len())));
    }
    let (local, domain) = raw
        .rsplit_once('@')
        .ok_or_else(|| GatewayError::Address(format!("no domain in {raw:?}")))?;
    if local.is_empty() || domain.is_empty() {
        return Err(GatewayError::Address(format!("empty local part or domain in {raw:?}")));
    }
    if local.len() > 64 {
        return Err(GatewayError::Address("local part longer than 64 bytes".into()));
    }
    if !domain.contains('.') || !domain.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '.') {
    ...
    if !local.chars().all(|c| c.is_ascii_graphic()) {
        return Err(GatewayError::Address("local part contains control or non-ASCII characters".into()));
    }
    Ok(Mailbox::Remote {
...
pub fn local_part_to_username(local: &str) -> Result<String, GatewayError> {
    ...
    match hashgram_app::mail::parse_address(&format!("@{base}")) {
        ...
        _ => Err(GatewayError::Address(format!("{local:?} is not a valid username"))),
    }
}
...
mod tests {
    ...
    fn local_addresses_map_to_usernames() {
        assert_eq!(classify("<Alice@Hashgram.IO>", D).unwrap(), Mailbox::Local("alice".into()));
        assert_eq!(classify("alice+news@hashgram.io", D).unwrap(), Mailbox::Local("alice".into()));
        assert_eq!(classify("@relay.example:bob@hashgram.io", D).unwrap(), Mailbox::Local("bob".into()));
        assert_eq!(classify("bob.b-2_x@hashgram.io", D).unwrap(), Mailbox::Local("bob.b-2_x".into()));
    }
    ...
    fn other_operator_domain() {
        assert_eq!(classify("carol@mail.example.org", "mail.example.org").unwrap(), Mailbox::Local("carol".into()));
        assert!(matches!(classify("carol@hashgram.io", "mail.example.org").unwrap(), Mailbox::Remote { .. }));
    }
}

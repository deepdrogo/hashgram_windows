...
#[derive(Parser, Debug)]
#[command(name = "hashgram-mail-gateway", about = "External SMTP gateway for HashMail", version)]
struct Cli {
    /// Path to `gateway.toml`.
    #[arg(short, long, default_value = "gateway.toml", env = "HASHGRAM_GATEWAY_CONFIG")]
    config: PathBuf,
    #[command(subcommand)]
    cmd: Cmd,
}
...
fn init_tracing() {
    let filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info,hickory_resolver=warn,hickory_proto=warn"));
...
async fn main() -> anyhow::Result<()> {
    init_tracing();
    let cli = Cli::parse();
    let cfg = GatewayConfig::load(&cli.config).with_context(|| format!("loading {}", cli.config.display()))?;
    match cli.cmd {
...
fn check_config(cfg: &GatewayConfig) -> anyhow::Result<()> {
    ...
    println!("  smtp listen:       {} (hostname {})", cfg.smtp.listen, cfg.smtp_hostname());
    ...
    println!("  http:              {}", if cfg.http.listen.is_empty() { "disabled" } else { &cfg.http.listen });
    match (&cfg.domain.dkim_selector, &cfg.domain.dkim_private_key_file) {
        ...
        _ => println!("  dkim:              DISABLED (outbound mail will be rejected by most receivers)"),
    }
    println!(
        "  policy:            spf/dkim required={} spam>{} contacts-only={}",
        cfg.policy.require_spf_or_dkim, cfg.policy.reject_spam_score_over, cfg.policy.allow_outbound_from_contacts_only
    );
    println!("  outbound:          port {} require_tls={} smarthost={:?}", cfg.outbound.smtp_port, cfg.outbound.require_tls, cfg.outbound.smarthost);
    let vault = hashgram_sdk::Paths::new(&cfg.hashgram.home).vault();
    println!("  vault present:     {}", vault.exists());
    println!("  passphrase env:    {}", if std::env::var(PASSPHRASE_ENV).is_ok() { "set" } else { "NOT SET" });
    Ok(())
}

fn dns(cfg: &GatewayConfig, mx_host: Option<&str>, ip4: &[String], ip6: &[String]) -> anyhow::Result<()> {
    ...
    match (&cfg.domain.dkim_selector, &cfg.domain.dkim_private_key_file) {
        ...
        _ => println!("; DKIM: configure domain.dkim_selector and domain.dkim_private_key_file first"),
    }
...
fn init(cfg: &GatewayConfig, device: &str) -> anyhow::Result<()> {
    ...
    if paths.vault().exists() {
        anyhow::bail!("{} already exists; refusing to overwrite the bridge identity", paths.vault().display());
    }
    std::fs::create_dir_all(&cfg.hashgram.home)?;
    let passphrase = GatewayConfig::passphrase()?;
    let kdf = cfg.sdk_config()?.kdf;
    let (account, mnemonic) = hashgram_sdk::account::Account::create(&paths.vault(), &passphrase, device, kdf)?;
    account.save()?;
    ...
    println!("  {mnemonic}");
    println!();
    println!("next: fund the address, then `hashgram-mail-gateway register`, then claim a username");
    println!("      (e.g. `gateway`) for it with hashgram-client so users can add it as a contact.");
    Ok(())
}
...
async fn register(cfg: &GatewayConfig, label: &str) -> anyhow::Result<()> {
    ...
    if one.link.peers().await.is_empty() {
        anyhow::bail!("no node completed the Hashgram handshake; check [hashgram] bootstrap / chain_api");
    }
    let r = hashgram_sdk::account::create_identity_on_chain(&one.account, &one.network, &one.chain, label, "server").await?;
